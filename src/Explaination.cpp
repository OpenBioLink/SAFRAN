...
Explaination::Explaination(std::string dbName, bool init) {
...
void Explaination::initDb() {
    ...
    sql = "CREATE TABLE Cluster(" \
        "ID INT     NOT NULL," \
        "PredictionTaskID INT NOT NULL," \
        "PredictionEntityID INT NOT NULL," \
        "Confidence REAL NOT NULL,"\
        "FOREIGN KEY(PredictionTaskID) REFERENCES Prediction(TaskID)," \
        "FOREIGN KEY(PredictionEntityID) REFERENCES Prediction(EntityID)," \
        "PRIMARY KEY(ID, PredictionTaskID, PredictionEntityID)" \
        ");";
    checkErrorCode(sqlite3_exec(db, sql, NULL, NULL, NULL), sql);

    sql = "CREATE TABLE Rule_Cluster(" \
        "RuleID INT NOT NULL," \
        "ClusterID INT NOT NULL," \
        "ClusterPredictionTaskID INT NOT NULL," \
        "ClusterPredictionEntityID INT NOT NULL," \
        "FOREIGN KEY(RuleID) REFERENCES Rule(ID)," \
        "FOREIGN KEY(ClusterID) REFERENCES Cluster(ID)," \
        "FOREIGN KEY(ClusterPredictionTaskID) REFERENCES Cluster(PredictionTaskID)," \
        "FOREIGN KEY(ClusterPredictionEntityID) REFERENCES Cluster(PredictionEntityID)," \
        "PRIMARY KEY(RuleID, ClusterID, ClusterPredictionTaskID, ClusterPredictionEntityID)" \
        ");";
    checkErrorCode(sqlite3_exec(db, sql, NULL, NULL, NULL), sql);
    */
...
void Explaination::insertEntities(Index* index) {
...
void Explaination::insertRelations(Index* index) {
...
void Explaination::insertRules(RuleReader* rr, int relsize, ClusteringReader* cr) {
    int* adj_begin = rr->getCSR()->getAdjBegin();
    Rule* rules_adj_list = rr->getCSR()->getAdjList();

    char* sql = "INSERT INTO Rule (ID,CLUSTER_ID,DEF,CONFIDENCE) VALUES (?,?,?,?);";
    sqlite3_stmt* stmt = prepare(sql);
    ...
    if (cr == nullptr) {
        for (int rel = 0; rel < relsize; rel++) {
            ...
            for (int i = 0; i < lenRules; i++) {
                ...
                checkErrorCode(sqlite3_bind_text(stmt, 3, rulestr.c_str(), strlen(rulestr.c_str()), 0));

                checkErrorCode(sqlite3_bind_double(stmt, 4, r.getAppliedConfidence()));
        ...
        std::unordered_map<int, std::pair<bool, std::vector<std::vector<int>>>> rel2clusters = cr->getRelToClusters();
        for (int rel = 0; rel < relsize; rel++) {
            int ind_ptr = adj_begin[3 + rel];
            int lenRules = adj_begin[3 + rel + 1] - ind_ptr;
            std::vector<std::vector<int>> cluster = rel2clusters[rel].second;
            for (int cluster_id = 0; cluster_id < cluster.size(); cluster_id++) {
                for (auto rule : cluster[cluster_id]) {
                    ...
                    checkErrorCode(sqlite3_bind_int(stmt, 2, cluster_id));
...
void Explaination::insertTask(int task_id, bool is_head, int relation_id, int entity_id) {
...
void Explaination::insertPrediction(int task_id, int entity_id, bool hit, double confidence) {
...
void Explaination::insertCluster(int task_id, int entity_id, int cluster_id, double confidence) {
...
void Explaination::insertRule_Cluster(int task_id, int entity_id, int cluster_id, int rule_id) {
...
void Explaination::insertRule_Entity(int rule_id, int task_id, int entity_id) {
    //std::cout << "Rule_Cluster " << task_id << " " << entity_id << " " << cluster_id << " " << rule_id << "\n";
...
void Explaination::checkErrorCode(int code, char* sql) {
    if (code != SQLITE_OK && code != SQLITE_DONE) {
        const char* err;
        if (this->db) {
            err = sqlite3_errmsg(this->db);
...
int Explaination::getNextTaskID() {
    int task_id_;
#pragma omp critical 
    {
        task_id++;

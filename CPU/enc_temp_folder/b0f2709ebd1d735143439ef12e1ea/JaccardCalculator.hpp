...
#pragma omp parallel for schedule(dynamic)
		for (int j = 0; j < len; j++) {
			...
			if (currRule.getRulestring().find("DRUG") == std::string::npos) {
			...
			for (int j = 0; j < len; j++) {
				if (i != j) {
					...
					*/
						...
						if (jaccard > 0.0 and rule_i.getRulestring() != rule_j.getRulestring() and rule_i.getRulestring().find("DRUG") != std::string::npos and rule_j.getRulestring().find("DRUG") != std::string::npos) {
							std::cout << rule_i.getRulestring() << "\n";
							std::cout << rule_j.getRulestring() << "\n"; 
							std::cout << "\n";
						}
					//}
				}
